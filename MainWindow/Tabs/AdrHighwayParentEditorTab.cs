...
namespace Klyte.Addresses.UI
{
    ...
    internal class AdrHighwayParentEditorTab : UICustomControl
    {
        ...
        private void OnEditLongPrefix(string x) { EditingInstance.FullPrefix = x; ReloadExamples(); }

        private void OnSetInvertConcatenateShort(bool x) { EditingInstance.InvertShortConcatenationOrder = x; ReloadExamples(); }

        private void OnSetInvertConcatenateLong(bool x) { EditingInstance.InvertFullConcatenationOrder = x; ReloadExamples(); }

        private void OnSetSpaceShort(bool x) { EditingInstance.AddSpaceBetweenTermsShort = x; ReloadExamples(); }

        private void OnSetSpaceLong(bool x) { EditingInstance.AddSpaceBetweenTermsFull = x; ReloadExamples(); }
        ...
        private string ExecuteItemChange(string targetValue, bool isValidSelection)
        {
            ...
            if (isValidSelection)
            {
                ...
                m_fullPrefix.text = EditingInstance.FullPrefix ?? "";
                m_shortAddSpace.isChecked = EditingInstance.AddSpaceBetweenTermsShort;
                m_fullAddSpace.isChecked = EditingInstance.AddSpaceBetweenTermsFull;
                m_fullInvert.isChecked = EditingInstance.InvertFullConcatenationOrder;
